...
*       _COLLECT       = ' '
  ...
  WHERE dummy_key = abap_true.
    IF sy-subrc <> 0.
      CLEAR rs_settings.
      rs_settings-dummy_key = abap_true.
    ENDIF.
  ENDMETHOD.
...
*         PARAMETER      =
  ...
  METHOD save.
    ##NEEDED DATA lv_dummy_key TYPE xfeld.
    SELECT SINGLE dummy_key INTO lv_dummy_key
      FROM zapp_settings
      WHERE dummy_key = is_settings-dummy_key.
    IF sy-subrc <> 0.
...
CLASS lcl_settings_user DEFINITION FINAL.
  PUBLIC SECTION.
    METHODS lock
      RAISING zcx_app_exception.
    ...
    METHODS get_settings
      RETURNING VALUE(rs_settings) TYPE zapp_settings_u.

    METHODS handle_exit
      IMPORTING is_settings TYPE zapp_settings_u
      RAISING   zcx_app_exception.
...
CLASS lcl_settings_user IMPLEMENTATION.

  METHOD lock.

    CALL FUNCTION 'ENQUEUE_EZAPP_SETTINGS_U'
      EXPORTING
*       MODE_ZAPP_SETTINGS_U       = 'E'
        uname          = sy-uname
*       X_UNAME        = ' '
        _scope         = '1'
*       _WAIT          = ' '
*       _COLLECT       = ' '
  ...
  METHOD unlock.
    CALL FUNCTION 'DEQUEUE_EZAPP_SETTINGS_U'.

  ENDMETHOD.

  METHOD get_settings.
    SELECT SINGLE * FROM zapp_settings_u INTO rs_settings
  WHERE uname = sy-uname.
    IF sy-subrc <> 0.
      CLEAR rs_settings.
      rs_settings-uname = sy-uname.
    ENDIF.
  ENDMETHOD.

  METHOD handle_exit.
    DATA ls_settings_old TYPE zapp_settings_u.
    DATA lv_answer TYPE c.
    ls_settings_old = get_settings( ).
    IF ls_settings_old <> is_settings.
      CALL FUNCTION 'POPUP_TO_CONFIRM'
...
*         PARAMETER      =
  ...
  METHOD save.
    ...
    SELECT SINGLE uname INTO lv_uname
      FROM zapp_settings_u
      WHERE uname = sy-uname.
    IF sy-subrc <> 0.
      INSERT zapp_settings_u FROM is_settings.
    ELSE.
      UPDATE zapp_settings_u FROM is_settings.
    ENDIF.
    COMMIT WORK AND WAIT.
  ENDMETHOD.
...
DATA gr_settings_user TYPE REF TO lcl_settings_user.
...
AT SELECTION-SCREEN.
  ...
  IF sy-ucomm = 'ONLI'.
    IF p_set_g = abap_true.
      ...
      ENDTRY.
    ELSE.
      CREATE OBJECT gr_settings_user.
      TRY.
          gr_settings_user->lock( ).
        CATCH zcx_app_exception INTO gr_ex. " Pretty Printer Exception
          MESSAGE gr_ex.
      ENDTRY.
    ENDIF.
...
START-OF-SELECTION.
  ...
  IF p_set_g = abap_true.
    ...
    gr_settings->unlock( ).
  ELSE.
    zapp_settings_u = gr_settings_user->get_settings( ).

    CALL SCREEN 0200.
    gr_settings_user->unlock( ).
  ENDIF.
